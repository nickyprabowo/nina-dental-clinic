<!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper" id="laporan-obat">
            ...
            <div class="box-body">
              ...
              <div class="col-sm-4">
                  <div class="input-group">
                    ...
                    <div class="input-group-addon btn btn-primary btn-flat" id="filterLaporanObat" @click="filterLaporanObat()">
                      <i class="fa fa-filter fa-fw"></i> Filter
                    </div>
					<button class="btn btn-primary btn-flat" onclick="window.open('<?php echo base_url()?>obat/printlaporan/'+$('#reservationtime').val(),'printlaporan','scrollbars=yes,status=no,menubar=no,width=800,height=400')" style="float:right;"><i class="fa fa-fw fa-print"></i></button>
                  </div><!-- /.input group -->
              </div>
              </div>
              ...
              <table id="daftar-obat" class="table table-bordered table-striped">
                <thead>
                  <tr>
                    ...
                    <th>Tanggal</th>
                    <th v-on:click="sortBy('stok')">Stok Sebelumnya</th>
                ...
                <tbody>
                  <tr v-for="laporan in paginatedItems | orderBy sortKey reverse" track-by="$index">
                    ...
                    <td v-if="laporan.edited"><input type="text" name="editPrevStokObat" v-model="laporan.prevStok"></td>
              ...
              <ul class="pagination">
                <li v-for="n in Math.ceil(laporanObat.length/per_page)" @click="current_page = $index" v-bind:class="{'active': $index === current_page}">
                  <a href="#">{{ $index + 1 }}</a>
                </li>
              </ul>
            </div><!-- /.box-body -->
